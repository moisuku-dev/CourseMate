...
# 입력 파일
INPUT_FILE = 'clean_review_성심당 본점.csv' 

# 출력 파일
OUTPUT_FILE = 'processed_reviews_성심당 본점.csv'


# --- (2/4) 4가지 작업을 위한 함수 정의 ---
def clean_text(text):
...
def get_sentiment(text, sentiment_pipeline):
    ...
    try:
        ...
        # 모델마다 라벨 이름이 다를 수 있어서(POSITIVE 또는 LABEL_1) 둘 다 체크
...
# 2. 데이터 로드
print(f"[2/5] 데이터 로딩: {INPUT_FILE} ('깨끗한' CSV) 읽는 중...")
try:
    ...
    print(f"  - 에러: {INPUT_FILE}을 찾을 수 없습니다. (먼저 'convert_to_csv.py'를 실행하세요)")
    sys.exit()
except Exception as e:
